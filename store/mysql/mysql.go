...
func (s *Store) ListEntities(ctx context.Context, f store.ListEntitiesFilter) (*store.ListEntitiesResult, error) {
	...
	if f.EntityType != "" {
		conds = append(conds, "ec.entity_type = ?")
	...
	if err := s.db.QueryRowContext(ctx, countQ, args...).Scan(&total); err != nil {
		return nil, fmt.Errorf("trailog/store/mysql: list entities count: %w", err)
	}

	// Cursor filter on the outer query.
	outerConds := []string{}
	cursorArgs := []any{}
	if f.Cursor != "" {
		...
		if len(parts) == 2 {
			...
			if err == nil {
				if sortDir == "ASC" {
					outerConds = append(outerConds, "(last_changed_at > ? OR (last_changed_at = ? AND entity_id > ?))")
				} else {
					outerConds = append(outerConds, "(last_changed_at < ? OR (last_changed_at = ? AND entity_id > ?))")
				}
				cursorArgs = append(cursorArgs, cursorTime.UTC(), cursorTime.UTC(), cursorID)
			}
		}
	}
	outerWhere := ""
	...
	// MySQL does not support ARRAY_AGG; use a correlated subquery for last-row values.
	mainQ := fmt.Sprintf(`
		...
		FROM (
			SELECT
				...
				(SELECT ec2.op FROM audit_entity_change ec2
				 JOIN audit_revision r2 ON r2.id = ec2.revision_id
				 WHERE ec2.entity_type = ec.entity_type AND ec2.entity_id = ec.entity_id
				 ORDER BY r2.occurred_at DESC LIMIT 1) AS last_op,
				(SELECT r2.actor_id FROM audit_revision r2
				 JOIN audit_entity_change ec2 ON ec2.revision_id = r2.id
				 WHERE ec2.entity_type = ec.entity_type AND ec2.entity_id = ec.entity_id
				 ORDER BY r2.occurred_at DESC LIMIT 1) AS last_actor_id,
				(SELECT r2.actor_name FROM audit_revision r2
				 JOIN audit_entity_change ec2 ON ec2.revision_id = r2.id
				 WHERE ec2.entity_type = ec.entity_type AND ec2.entity_id = ec.entity_id
				 ORDER BY r2.occurred_at DESC LIMIT 1) AS last_actor_name
			FROM audit_entity_change ec
			JOIN audit_revision r ON r.id = ec.revision_id
			%s
			GROUP BY ec.entity_type, ec.entity_id
		...
		LIMIT ?`, where, outerWhere, sortCol, sortDir)

	queryArgs := append(append([]any{}, args...), cursorArgs...)
	queryArgs = append(queryArgs, limit)

	rows, err := s.db.QueryContext(ctx, mainQ, queryArgs...)
	if err != nil {
		return nil, fmt.Errorf("trailog/store/mysql: list entities: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.EntityType, &e.EntityID, &e.LastChangedAt,
			...
			return nil, fmt.Errorf("trailog/store/mysql: scan entity summary: %w", err)
		}
...
func (s *Store) GetWebhookTargetConfig(ctx context.Context, entityType string) (*store.WebhookTargetConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("trailog/store/mysql: get webhook target config: %w", err)
	}
	c.Auth = auth.String
	return &c, nil
}
...
func (s *Store) SaveWebhookTargetConfig(ctx context.Context, cfg store.WebhookTargetConfig) error {
	...
	if err != nil {
		return fmt.Errorf("trailog/store/mysql: save webhook target config: %w", err)
	}
	return nil
}
...
func (s *Store) DeleteWebhookTargetConfig(ctx context.Context, entityType string) error {
	const q = `DELETE FROM audit_webhook_target_config WHERE entity_type = ?`
	if _, err := s.db.ExecContext(ctx, q, entityType); err != nil {
		return fmt.Errorf("trailog/store/mysql: delete webhook target config: %w", err)
	}
	return nil
}
...
func (s *Store) DeleteRevision(ctx context.Context, id string) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("trailog/store/mysql: delete revision begin tx: %w", err)
	}
	defer func() {
		if err != nil {
			_ = tx.Rollback()
	...
	if _, err = tx.ExecContext(ctx, `
		...
		WHERE aec.revision_id = ?`, id); err != nil {
		return fmt.Errorf("trailog/store/mysql: delete field diffs: %w", err)
	}
	if _, err = tx.ExecContext(ctx,
		`DELETE FROM audit_entity_change WHERE revision_id = ?`, id); err != nil {
		return fmt.Errorf("trailog/store/mysql: delete entity changes: %w", err)
	}
	if _, err = tx.ExecContext(ctx,
		`DELETE FROM audit_revision WHERE id = ?`, id); err != nil {
		return fmt.Errorf("trailog/store/mysql: delete revision: %w", err)
	}
	return tx.Commit()
}
