...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revision_correlation' AND object_id = OBJECT_ID('audit_revision'))
    CREATE INDEX idx_revision_correlation ON audit_revision (correlation_id);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revision_occurred_at' AND object_id = OBJECT_ID('audit_revision'))
    CREATE INDEX idx_revision_occurred_at ON audit_revision (occurred_at DESC);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revision_actor' AND object_id = OBJECT_ID('audit_revision'))
    CREATE INDEX idx_revision_actor ON audit_revision (actor_id);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revision_action' AND object_id = OBJECT_ID('audit_revision'))
...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_entity_change_entity' AND object_id = OBJECT_ID('audit_entity_change'))
    CREATE INDEX idx_entity_change_entity ON audit_entity_change (entity_type, entity_id, created_at DESC);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_entity_change_revision' AND object_id = OBJECT_ID('audit_entity_change'))
...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_field_diff_entity_change' AND object_id = OBJECT_ID('audit_field_diff'))
    CREATE INDEX idx_field_diff_entity_change ON audit_field_diff (entity_change_id);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_field_diff_field_name' AND object_id = OBJECT_ID('audit_field_diff'))
...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_relation_parent' AND object_id = OBJECT_ID('audit_relation'))
    CREATE INDEX idx_relation_parent ON audit_relation (parent_type, parent_id);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_relation_child' AND object_id = OBJECT_ID('audit_relation'))
...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revert_log_target' AND object_id = OBJECT_ID('audit_revert_log'))
    CREATE INDEX idx_revert_log_target ON audit_revert_log (target_revision_id);
-- +goose StatementEnd
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_revert_log_created' AND object_id = OBJECT_ID('audit_revert_log'))
...
-- +goose StatementBegin
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_outbox_undelivered' AND object_id = OBJECT_ID('audit_outbox'))
...
-- +goose Down
-- +goose StatementBegin
IF OBJECT_ID('audit_outbox',      'U') IS NOT NULL DROP TABLE audit_outbox;
-- +goose StatementEnd
-- +goose StatementBegin
IF OBJECT_ID('audit_revert_log',  'U') IS NOT NULL DROP TABLE audit_revert_log;
-- +goose StatementEnd
-- +goose StatementBegin
IF OBJECT_ID('audit_field_diff',  'U') IS NOT NULL DROP TABLE audit_field_diff;
-- +goose StatementEnd
-- +goose StatementBegin
IF OBJECT_ID('audit_entity_change','U') IS NOT NULL DROP TABLE audit_entity_change;
-- +goose StatementEnd
-- +goose StatementBegin
IF OBJECT_ID('audit_relation',    'U') IS NOT NULL DROP TABLE audit_relation;
-- +goose StatementEnd
-- +goose StatementBegin
IF OBJECT_ID('audit_revision',    'U') IS NOT NULL DROP TABLE audit_revision;
-- +goose StatementEnd
